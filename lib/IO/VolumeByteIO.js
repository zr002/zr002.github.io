...
export default class VolumeByteIO {
  ...
  static async read(filename) {
    ...
    // Update dimensions for the engine block volume (example: 256x256x128)
    const dims = [256, 256, 44];
    const sizes = [1, 1.5, 1.5];
    const data = [];
    let offset = 0;
    let view = new DataView(binarydata);
    for (let i = 0; i < dims[0] * dims[1] * dims[2]; ++i) {
      let value = view.getUint8(offset, true);
      data.push(value);
      offset += 1;
    }
