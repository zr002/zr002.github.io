...
export default class VolumeRenderingSimpleObject extends RayTracingObject {
  constructor(device, canvasFormat, camera) {
    ...
    this._volume = new VolumeData('/assets/frog_256x256x44_uint8.raw');
    this._camera = camera;
  }
  ...
  async createGeometry() {
    ...
    // Copy from CPU to GPU - note we use the offset to copy one by one
    this._device.queue.writeBuffer(this._dataBuffer, 0, new Float32Array(this._volume._data));
    
    // *** NEW: Create transfer function uniform buffer (4 bytes) ***
    this._tfBuffer = this._device.createBuffer({
      label: "Transfer Function Buffer " + this.getName(),
      size: 4,
      usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST,
    });
    //this._device.queue.writeBuffer(this._tfBuffer, 0, new Uint32Array([1]));

    // IMPORTANT: Call createShaders() before creating the compute pipeline.
    await this.createShaders();
    await this.createComputePipeline();
    // Remove the call that uses undefined offScreenTexture:
    // this.createBindGroup(offScreenTexture);
    // Instead, let the RayTracer (in setTracerObject) call createBindGroup with its valid texture.
  }
  ...
  async createShaders() {
    ...
    this._bindGroupLayout = this._device.createBindGroupLayout({
      ...
      { 
        binding: 4,
        visibility: GPUShaderStage.COMPUTE,
        buffer: {} // Transfer function uniform buffer
      }]
    });
  ...
  createBindGroup(outTexture) {
    ...
    this._bindGroup = this._device.createBindGroup({
      ...
      entries: [
        ...
        { binding: 4, resource: { buffer: this._tfBuffer } }
      ],
    });
