...
// --------------------------------------------------------------------
// NEW: Add a shading uniform struct (for mode, specularPower, etc.)
// --------------------------------------------------------------------
struct ShadingUniform {
  mode           : f32, // 0=Lambertian, 1=Phong, 2=Toon
  specularPower  : f32, // e.g. 32.0 for Phong
  toonLevels     : f32, // e.g. 5.0 for Toon shading
  filler         : f32, // extra float for alignment (unused, or ambient factor)
};

// NEW: Bind group for shading at binding=4
@group(0) @binding(4) var<uniform> shading : ShadingUniform;
...
fn boxEmitColor() -> vec4f {
  return vec4f(0.2, 0.2, 0.2, 1); // my box doesn't emit any color
}
...
fn getLightInfo(lightPos: vec3f, lightDir: vec3f, hitPoint: vec3f, objectNormal: vec3f) -> LightInfo {
  // We'll store the result in out
  var out: LightInfo;
  
  // 1) read the "lightType" from light.params[3]
  let lt = light.params[3]; // 0=point, 1=directional, 2=spot
  ...
  if (lt < 0.5) {
    ...
    // =========== DIRECTIONAL LIGHT =========== 
    // No distance attenuation, rays are parallel to lightDir
    var intensity = light.intensity; // we do not reduce with distance
    // direction from surface to light is just the opposite of lightDir if you want it
    var dirNorm = normalize(lightDir);
    let viewDirection = -dirNorm; // direction from surface to the "sun"
    // dot with normal
    let ndl = max(dot(viewDirection, -objectNormal), 0);
    out.intensity = intensity * ndl;
    out.lightdir  = viewDirection;

  } else {
    // =========== SPOT LIGHT =========== 
    // combine distance attenuation + cone cutoff
    let cutoff  = light.params[0];  // from your SpotLight constructor
    let dropoff = light.params[1];  // from your SpotLight constructor
    ...
    var viewDirection = normalize(hitPoint - lightPos);
    // check angle
    let d = dot(normalize(lightDir), viewDirection);
    let c = cos(cutoff);
    if (d < c) {
      ...
      // inside => apply dropoff
      let spotFactor = pow(d, dropoff);
      intensity *= spotFactor;
      // also dot with normal
      let ndl = max(dot(viewDirection, -objectNormal), 0);
      out.intensity = intensity * ndl;
      out.lightdir  = viewDirection;
    }
...
// --------------------------------------------------------------------
// NEW: Helper for Phong specular
// --------------------------------------------------------------------
fn computeSpecular(viewDir: vec3f, lightDir: vec3f, normal: vec3f, specPower: f32) -> f32 {
  // In Phong: R = reflect(-lightDir, normal)
  // spec = (dot(R, viewDir))^specPower if >0 else 0
  let R = reflect(-lightDir, normal);
  let rd = dot(R, viewDir);
  if (rd > 0.0) {
    return pow(rd, specPower);
  }
  return 0.0;
}

// --------------------------------------------------------------------
// NEW: Helper to quantize a float into discrete toon levels
// --------------------------------------------------------------------
fn toonQuant(value: f32, levels: f32) -> f32 {
  // For example: if levels=5, we map value into one of 0, 0.25, 0.5, 0.75, 1.0
  // We'll clamp the value [0..1] then find an index, then convert back.
  let vclamp = clamp(value, 0.0, 1.0);
  let idx = floor(vclamp * levels);
  return idx / (levels - 1.0);
}
...
fn computeOrthogonalMain(@builtin(global_invocation_id) global_id: vec3u) {
  ...
  if (uv.x < texDim.x && uv.y < texDim.y) {
    ...
    if (hitInfo.x > 0) { // if there is a hit
      ...
      // --- NEW LINES FOR MULTI-SHADING ---
      // We'll retrieve shading mode, specular, etc.
      let mode        = shading.mode;           // 0=Lambert, 1=Phong, 2=Toon
      let specPow     = shading.specularPower;  // e.g. 32.0
      let toonLevels  = shading.toonLevels;     // e.g. 5.0

      // We'll also need a view direction for Phong/Toon spec:
      // In orthographic, you could use (0,0,-1), or a real camera position:
      let camPos = applyMotorToPoint(vec3f(0.0, 0.0, 0.0), cameraPose.motor);
      let viewDir = normalize(camPos - hitPt);

      var finalColor = vec4f(0);
      if (mode < 0.5) {
        // Lambertian
        finalColor = emit + diffuse * lightInfo.intensity;
      } else if (mode < 1.5) {
        // Phong
        let kd = diffuse.xyz; 
        let Idiff = kd * lightInfo.intensity.xyz;
        let spec = computeSpecular(viewDir, lightInfo.lightdir, normal, specPow);
        // basic white highlight:
        let ks = vec3f(1.0, 1.0, 1.0) * spec;
        finalColor = vec4f(emit.xyz + Idiff + ks * lightInfo.intensity.xyz, 1.0);
      } else {
        // Toon
        let kd = diffuse.xyz;
        let Idiff = dot(normal, -lightInfo.lightdir);
        if (Idiff < 0.0) {
          finalColor = emit;
        } else {
          let quantDiff = toonQuant(Idiff, toonLevels);
          let rawSpec = computeSpecular(viewDir, lightInfo.lightdir, normal, specPow);
          let quantSpec = toonQuant(rawSpec, toonLevels);
          var toonRGB = emit.xyz
            + kd * quantDiff * lightInfo.intensity.xyz
            + vec3f(1.0,1.0,1.0) * quantSpec * lightInfo.intensity.xyz;
          finalColor = vec4f(toonRGB, 1.0);
        }
      }
      // --- END NEW SHADING LINES ---

      // Overwrite color with final result
      color = finalColor;
    }
...
fn computeProjectiveMain(@builtin(global_invocation_id) global_id: vec3u) {
  ...
  if (uv.x < texDim.x && uv.y < texDim.y) {
    let ndc = vec2f(
      (f32(uv.x) + 0.5) / f32(texDim.x) * 2.0 - 1.0,
      (f32(uv.y) + 0.5) / f32(texDim.y) * 2.0 - 1.0
    );

    var spt  = vec3f(0.0, 0.0, 0.0);
    var rdir = vec3f(ndc.x, ndc.y, 1.0);
    ...
    if (hitInfo.x > 0.0) {
      ...
      // read shading params
      let mode       = shading.mode;
      let specPow    = shading.specularPower;
      let toonLevels = shading.toonLevels;
