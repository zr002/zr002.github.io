...
export default class Polygon {
  ...
  isInside(v0, v1, p) {
    ...
    // NEW METHOD: Basic point-in-polygon test for CONVEX shapes.
  // Returns true if the point (px, py) is inside the convex polygon.
  isInsideConvex(px, py) {
    for (let i = 0; i < this._numV - 1; i++) {
      const v0 = this._polygon[i];
      const v1 = this._polygon[i + 1];
      // Compute the cross product between the edge (v1 - v0) and (point - v0)
      const cross = (v1[0] - v0[0]) * (py - v0[1]) - (v1[1] - v0[1]) * (px - v0[0]);
      if (cross < 0) {
        return false;  // if point is on the "right" of any edge, it is outside.
      }
    }
    return true;
  }
  
  // NEW METHOD: Winding number test for NON-CONVEX polygons.
  // Returns true if the point (px, py) is inside the polygon.
  isInsideWinding(px, py) {
    let windingNumber = 0;
    for (let i = 0; i < this._numV - 1; i++) {
      const v0 = this._polygon[i];
      const v1 = this._polygon[i + 1];
      // Check if the edge crosses the horizontal line at py.
      const crossesUp = (v0[1] <= py && v1[1] > py);
      const crossesDown = (v0[1] > py && v1[1] <= py);
      if (crossesUp || crossesDown) {
        const intersectX = v0[0] + (py - v0[1]) * (v1[0] - v0[0]) / (v1[1] - v0[1]);
        if (intersectX > px) {
          // Increment for upward crossing, decrement for downward.
          if (v1[1] > v0[1]) {
            windingNumber++;
          } else {
            windingNumber--;
