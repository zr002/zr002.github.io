...
fn computeProjectiveMain(@builtin(global_invocation_id) global_id: vec3u) {
  ...
  if (uv.x < texDim.x && uv.y < texDim.y) {
    ...
    //    to set up the ray in camera space. 
    //    Suppose the camera is located at (0,0,0) in camera space.
    //    We'll assume a pinhole at Z=0, projecting forward along -Z:
    let focalDist = cameraPose.focal.x;  // or .y, whichever your camera logic uses

    // The ray start is at the origin of camera space
    var rayStart = vec3f(0.0, 0.0, 0.0);

    // The direction is from the pinhole through the pixel on the focal plane
    // If we assume negative Z is forward, let's put that plane at Z = -focalDist:
    var rayDir = normalize(vec3f(ndcX, ndcY, -focalDist));

    //    using the same transformPt / transformDir as orthographic code
    rayStart = transformPt(rayStart);
    rayDir   = transformDir(rayDir);

    traceScene(uv, rayStart, rayDir);
  }
}
